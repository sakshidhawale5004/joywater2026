  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          ...
          { name: "description", content: `${loaderData.product.name} in ${loaderData.product.finish}. Handcrafted by Joy Water.` },
          { property: "og:title", content: `${loaderData.product.name} — Joy Water` },
          { property: "og:description", content: `Handcrafted ${loaderData.product.finish} finish.` },
        ]
      : [],
  }),
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <div className="max-w-3xl mx-auto px-8 py-32 text-center">
        ...
        <Link to="/products" className="text-gold underline">All products</Link>
      </div>
    </SiteLayout>
  ),
});
      ...
      <div className="max-w-7xl mx-auto px-8 py-12 grid md:grid-cols-2 gap-12">
        <div className={`aspect-square bg-gradient-to-br ${gradient} relative overflow-hidden`}>
          ...
          <svg viewBox="0 0 200 200" className="w-full h-full text-white/70 mix-blend-overlay p-16">
            <path d="M60 40 v40 a20 20 0 0 0 40 0 v-40" fill="none" stroke="currentColor" strokeWidth="2"/>
            <rect x="70" y="80" width="60" height="90" fill="none" stroke="currentColor" strokeWidth="2" rx="4"/>
          </svg>
        </div>
        <div className="flex flex-col justify-center">
          {cat && (
            <Link to="/category/$slug" params={{slug: cat.slug}} className="text-xs uppercase tracking-[0.3em] text-gold hover:underline">{cat.title}</Link>
          )}
          ...
          <p className="font-serif text-3xl text-primary mt-8">₹ {product.price.toLocaleString("en-IN")}</p>
          <dl className="grid grid-cols-2 gap-4 mt-10 pt-8 border-t border-border">
            <div><dt className="text-xs uppercase tracking-widest text-muted-foreground">Finish</dt><dd className="mt-1">{product.finish}</dd></div>
            <div><dt className="text-xs uppercase tracking-widest text-muted-foreground">Material</dt><dd className="mt-1">Solid Brass</dd></div>
            <div><dt className="text-xs uppercase tracking-widest text-muted-foreground">Warranty</dt><dd className="mt-1">10 Years</dd></div>
            <div><dt className="text-xs uppercase tracking-widest text-muted-foreground">Origin</dt><dd className="mt-1">India</dd></div>
          </dl>
          <Link to="/contact" className="mt-10 inline-flex justify-center bg-primary text-primary-foreground py-4 text-xs uppercase tracking-[0.3em] hover:bg-gold hover:text-primary transition-all">Enquire About This Piece</Link>
        </div>
      </div>
