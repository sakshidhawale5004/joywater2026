...
export const Route = createFileRoute("/sigma-series")({
  component: SigmaSeriesPage,
  head: () => ({
    meta: [
      { title: "Sigma Series — Joy Water" },
      {
        name: "description",
        content:
          "Premium Sigma Series bathroom accessories in Chrome, PVD Gold, and PVD Rose Gold finishes. Solid brass construction with superior quality.",
      },
      { property: "og:title", content: "Sigma Series — Joy Water" },
      { property: "og:description", content: "Complete Sigma Series bathroom accessories collection." },
    ],
  }),
});

function SigmaSeriesPage() {
  ...
  const filtered = useMemo(() => {
    if (finish === "all") {
      return sigmaSeriesProducts;
    }
    return sigmaSeriesProducts.filter((p) => p.finish === finish);
  }, [finish]);
  ...
  return (
    <SiteLayout>
      <SeriesHero
        ...
        tagline="Explore our complete Sigma Series collection — Chrome, PVD Gold, and PVD Rose Gold — designed to elevate your bathroom with modern elegance."
        productCount={sigmaSeriesProducts.length}
        features={["Solid Brass", "Multiple Finishes", "Modern Design", "11 Years Warranty"]}
